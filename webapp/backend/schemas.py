...
class SummerSlotPlacementInfo(BaseModel):
    """Nested placement summary in slot response."""
    id: int
    application_id: int
    student_name: str
    grade: str
...
class SummerSlotResponse(BaseModel):
    """Slot with tutor name and placement summary."""
    id: int
    config_id: int
    slot_day: str
    time_slot: str
    ...
    placements: List[SummerSlotPlacementInfo] = []

    model_config = ConfigDict(from_attributes=True)


# ---- Summer Placement Schemas ----

class SummerPlacementCreate(BaseModel):
    """Assign a student (application) to a slot."""
    application_id: int
    slot_id: int


class SummerPlacementUpdate(BaseModel):
    """Update placement status."""
    placement_status: Literal["Tentative", "Confirmed", "Cancelled"]


class SummerPlacementResponse(BaseModel):
    """Full placement response with joined data."""
    id: int
    application_id: int
    slot_id: int
...
class SummerApplicationResponse(BaseModel):
    ...
    reviewed_at: Optional[datetime] = None
    form_language: Optional[str] = None
    placements: List["SummerApplicationPlacementInfo"] = []
...
# ---- Summer Application Placement Info (for embedding in application response) ----

class SummerApplicationPlacementInfo(BaseModel):
    """Placement info embedded in application response."""
    id: int
    slot_id: int
    slot_day: str
    time_slot: str
...
# ---- Summer Session Schemas ----

class SummerSessionResponse(BaseModel):
    """Materialized session instance response."""
    id: int
    slot_id: int
    session_date: date
    lesson_number: int
...
class SummerSessionUpdate(BaseModel):
    """Update a session's lesson number, status, or notes."""
    lesson_number: Optional[int] = Field(None, ge=1, le=20)
    session_status: Optional[Literal["Scheduled", "Attended", "Cancelled"]] = None
    notes: Optional[str] = None


class SummerSessionCalendarEntry(BaseModel):
    """Session with slot info and placements for calendar view."""
    session_id: int
    slot_id: int
    slot_day: str
    time_slot: str
...
class SummerSessionCalendarResponse(BaseModel):
    """Calendar view data for one week."""
    week_start: date
    week_end: date
    sessions: List[SummerSessionCalendarEntry]


class SummerFindSlotResult(BaseModel):
    """A candidate session returned by find-slot search."""
    session_id: int
    slot_id: int
    date: date
    time_slot: str
