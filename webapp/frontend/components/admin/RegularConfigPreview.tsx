...
import {
  useRegularApplyFormState,
  FROZEN_SETTERS,
} from "@/hooks/useRegularApplyFormState";
...
function PreviewStepBody({
  ...
  lang: Lang;
  previewStep: number;
  initial: Parameters<typeof useRegularApplyFormState>[0];
}) {
  const form = useRegularApplyFormState(initial);
  switch (previewStep) {
    case 1:
      return (
        <StudentInfoStep
...
interface RegularConfigPreviewProps {
  config: RegularCourseFormConfig;
  previewStep: number;
  onStepChange: (step: number) => void;
}

export const RegularConfigPreview = memo(function RegularConfigPreview({
  config,
  previewStep,
  onStepChange,
}: RegularConfigPreviewProps) {
  ...
  // Sample centre for the selected student type so Step 2's radio state and
  // the Step 4 summary have something to show. "None" seeds no centre.
  const sampleCenter =
    previewStudentType && previewStudentType !== "None"
      ? config.center_options?.find((c) =>
          c.name_en.startsWith(previewStudentType)
        )?.name || ""
      : "";

  // Which controls to show per step (step 4 summary reflects both choices)
  const showLocationControl = previewStep === 3 || previewStep === 4;
  const showStudentTypeControl = previewStep === 2 || previewStep === 4;
  ...
  return (
    <div className="summer-light text-foreground bg-background flex flex-col h-full">
      ...
      <div className="flex items-center gap-1 px-3 py-2 border-b border-gray-200 bg-white shrink-0">
        {REGULAR_STEP_LABELS.map((label, i) => {
          const step = i + 1;
        ...
        <div className="pointer-events-none select-none p-4 sm:p-6">
          <PreviewStepBody
            ...
            initial={{
              ...
              currentCenters: sampleCenter ? [sampleCenter] : [],
