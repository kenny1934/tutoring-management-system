...
import {
  BRANCH_INFO, LOCATION_TO_CODE,
  REGULAR_EXIT_STATUSES, schoolGroupKey,
...
export function RegularApplicationStats({ applications, filters, readOnly = false, onAliasCreated }: Props) {
  ...
  // group is labelled with its most common original spelling, so `spellings`
  // is only tracked for those.
  const schoolData = useMemo(() => {
    const groups = new Map<string, {
      count: number; recognised: boolean; spellings: Map<string, number>;
    }>();
    let unknown = 0;
    for (const app of activeApps) {
      ...
      if (!group) {
        group = { count: 0, recognised: !!app.school_canonical, spellings: new Map() };
    ...
    const entries = Array.from(groups.entries())
      .map(([key, { count, recognised, spellings }]) => ({
        key, count, recognised,
        display: recognised
          ? key
          : Array.from(spellings.entries())
              .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))[0][0],
      }))
      .sort((a, b) => b.count - a.count || a.display.localeCompare(b.display));
    const max = entries.length > 0 ? entries[0].count : 0;
    return { entries, max, unknown };
  }, [activeApps]);
  ...
  // Counted over every application, exits included, unlike the active-only
  // charts: an assignment is global, and a spelling that arrived on a since-
  // withdrawn application will come back on the next one.
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      {/* Feeder schools */}
      <ChartCard title="Feeder Schools" badge={`${activeApps.length} active`} className="lg:col-span-2">
          ...
          <div className="space-y-1.5">
            {shownSchools.map(({ key, display, count, recognised }) => (
              <BarRow
                ...
                count={count} total={activeApps.length} maxCount={schoolData.max}
      ...
      <ChartCard title="Preferred Location" badge={`${activeApps.length} active`}>
          ...
          <div className="space-y-1.5">
            {locationData.entries.map(([code, count]) => (
              <BarRow
                ...
                count={count} total={activeApps.length} maxCount={locationData.max}
