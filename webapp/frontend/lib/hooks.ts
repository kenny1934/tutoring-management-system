...
/**
 * Hook for fetching the tutor x month revenue matrix for a calendar year.
 * Pass `enabled = false` to skip fetching (e.g. when the user is not an admin).
 */
export function useTutorYearMatrix(year: number, location: string | null | undefined, enabled: boolean = true) {
  const locationKey = location && location !== "All Locations" ? location : null;
  return useSWR<TutorYearMatrixResponse>(
    enabled ? ['revenue-year-matrix', year, locationKey ?? 'all'] : null,
    () => revenueAPI.getTutorYearMatrix(year, locationKey),
    { revalidateOnFocus: false }
  );
}
...
/**
 * Hook for the students the grade check set aside this school year. Only
 * fetched for admin viewers, since the endpoint refuses everyone else.
 */
export function useCurriculumGradeCheck(enabled: boolean) {
  return useSWR<CurriculumGradeCheckResponse>(
    enabled ? 'curriculum-grade-check' : null,
    () => curriculumAPI.getGradeCheck(),
    { revalidateOnFocus: false }
  );
}
...
/**
 * Hook for fetching aged pending makeup count (for notification bell)
 * Polls every 30 seconds when tab is visible
 */
export function useAgedPendingMakeupsCount(tutorId: number | null | undefined) {
  const refreshInterval = useVisibilityAwareInterval(30000);
  return useSWR<AgedPendingMakeupsCount>(
