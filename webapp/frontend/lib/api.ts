...
export const summerAPI = {
  ...
  createPlacement: (data: SummerPlacementCreate) =>
    fetchAPI<SummerPlacement>("/summer/placements", {
      method: "POST",
      body: JSON.stringify(data),
    }),

  updatePlacement: (id: number, data: SummerPlacementUpdate) =>
    fetchAPI<SummerPlacement>(`/summer/placements/${id}`, {
      method: "PATCH",
      body: JSON.stringify(data),
    }),

  deletePlacement: (id: number) =>
    fetchAPI<{ success: boolean }>(`/summer/placements/${id}`, { method: "DELETE" }),

  bulkConfirmPlacements: (configId: number, location?: string) => {
    const params = new URLSearchParams({ config_id: String(configId) });
    if (location) params.set("location", location);
    return fetchAPI<{ confirmed: number }>(`/summer/placements/bulk-confirm?${params}`, {
      method: "POST",
    });
  },
  ...
  autoSuggest: (data: { config_id: number; location: string }) =>
...
export const summerAPI = {
  ...
  generateSessions: (configId: number, location: string, slotId?: number) => {
    const params = new URLSearchParams({ config_id: String(configId), location });
    if (slotId) params.set("slot_id", String(slotId));
    return fetchAPI<{ success: boolean; sessions_created: number; slots_skipped: number }>(
      `/summer/sessions/generate?${params}`,
      { method: "POST" }
    );
  },

  getSessions: (slotId: number) =>
    fetchAPI<SummerSession[]>(`/summer/sessions?slot_id=${slotId}`),

  updateSession: (sessionId: number, data: SummerSessionUpdate) =>
    fetchAPI<SummerSession>(`/summer/sessions/${sessionId}`, {
      method: "PATCH",
      body: JSON.stringify(data),
    }),

  getSessionCalendar: (configId: number, location: string, weekStart: string) =>
    fetchAPI<SummerSessionCalendarResponse>(
      `/summer/sessions/calendar?config_id=${configId}&location=${encodeURIComponent(location)}&week_start=${weekStart}`
    ),
  ...
  findSlot: (params: {
    ...
    return fetchAPI<SummerFindSlotResult[]>(`/summer/sessions/find-slot?${qs}`);
  },
};
...
export const summerAPI = {
  ...
  deleteSession: (id: number) =>
    fetchAPI<{ success: boolean }>(`/summer/sessions/${id}`, { method: "DELETE" }),
...
export const prospectsAPI = {
  ...
  autoMatch: (year: number) =>
    fetchAPI<{ matched: number; total_unlinked: number }>(`/prospects/admin/auto-match?year=${year}`, {
      method: "POST",
    }),
};
