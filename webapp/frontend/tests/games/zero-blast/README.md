...
| `zb-multi-test.js` | 194 | host + 2 phones over a mocked GameBridge: lobby, kick (lobby + mid-game), claims, echo, grace, resume, tutor bar, restart revival, re-scan identity, name dedupe, sent·marking beat; then 探究模式 (等式開口中): the three-stage inquiry arc, secret pairing, the 0×0 boom, the bridge's one-equation-one-x judging (hint factors, the 或-note), the rotating bye on odd headcounts, tutor controls (加多一回合, stage jump), host F5 recovery mid-arc, handover to the main game |
