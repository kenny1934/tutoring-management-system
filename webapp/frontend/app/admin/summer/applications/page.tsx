...
export default function SummerApplicationsPage() {
  ...
  const [batchPublishResults, setBatchPublishResults] = useState<SummerPublishResult[] | null>(null);
  ...
  // Optimistic cache patch — side actions that mutate one application
  // (mark fee sent, unpublish, etc.) call this so the SWR cache reflects
  // the change before the server refetch lands. `revalidate: false` here
  // because the caller typically also triggers handleRefresh() right after
  // to reconcile against the server; applying both in parallel would cause
  // the optimistic patch to flicker.
  const optimisticallyUpdateApp = useCallback(
    (id: number, patch: Partial<SummerApplication>) => {
      if (!swrKey) return;
      mutate(
        swrKey,
        (current: SummerApplication[] | undefined) =>
  ...
  const handleBatchPublish = async () => {
    ...
    try {
      const resp = await summerAPI.publishApplicationsBatch(Array.from(checkedIds));
      ...
      if (resp.failed_count === 0) {
        ...
        showToast(
          `Published ${resp.published_count}, failed ${resp.failed_count}`,
          "error",
        );
      }
    ...
    <DeskSurface fullHeight>
      <PageTransition className="flex flex-col h-full p-4 sm:p-6">
        ...
        <div className="flex flex-col h-full bg-[#faf8f5] dark:bg-[#1a1a1a] rounded-xl border border-[#e8d4b8] dark:border-[#6b5a4a] shadow-sm paper-texture overflow-hidden">
              ...
              >
                ...
                <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-lg px-4 py-3 flex items-center gap-3">
                  ...
                  <button
                    ...
                    title="Publish selected applications to native enrollments. Each runs independently — failures don't block successes."
          ...
          {/* Batch publish results modal — shown after a batch publish completes
              so admin can inspect which apps failed and why (per app error_code). */}
          {batchPublishResults && (
            <div
