...
function TutorCard({ tutor, onOpen }: { tutor: Tutor; onOpen: () => void }) {
  ...
  return (
    <button
      onClick={onOpen}
      className="group text-left flex items-center gap-4 p-4 rounded-xl border border-[#e8d4b8] dark:border-[#6b5a4a] bg-white dark:bg-[#221c12] hover:shadow-md hover:border-[#d4a574] transition-all"
    >
      {picture ? (
        <Image
          src={picture}
