      ...
      {ownMembers.length > 0 && (
        <div className="sticky top-0 z-10 bg-background/95 backdrop-blur-sm -mx-4 px-4 sm:-mx-8 sm:px-8 py-3 border-b border-border">
          <div className="flex gap-2">
            ...
            {stats.crossBranch > 0 && (
              <button
                onClick={() => setFilterTab(filterTab === "cross-branch" ? "all" : "cross-branch")}
                className={`px-2.5 py-2 text-[11px] font-medium rounded-xl shrink-0 transition-colors ${
                  filterTab === "cross-branch"
                    ? "bg-amber-500 text-white border-2 border-amber-500"
                    : "border-2 border-amber-500/30 text-amber-600 hover:border-amber-500/50 hover:bg-amber-500/5"
                }`}
              >
                Cross-branch <span className={`inline-flex items-center justify-center min-w-[18px] h-[18px] text-[10px] font-bold rounded-full px-1 ${filterTab === "cross-branch" ? "bg-white/30" : "bg-amber-500/20"}`}>{stats.crossBranch}</span>
              </button>
            )}
