...
* **One list of contact types, and a type for course renewal**: the list of contact types offered when logging a parent contact had been copied into nine different places, so the wording drifted between them and adding a type meant finding all nine. There is one list now, and it has gained Course Renewal, so calls about September are recorded as what they are rather than as something else.


### Bug Fixes

* **The P6 Prospects page now refuses people who cannot read it**: every other admin page tells somebody without access that they do not have it, and this one drew its heading, its filters and an empty table instead. None of the records were ever at risk, because the data behind that page has always been admin only, but the page looked broken rather than closed.
...
* **The prospect journey chip opens the prospect's record**: the chip on a September application card read like a label, so the primary tutor's remark behind it could only be reached by leaving the page and searching the prospects list. That remark is often the reason an application is worth a second look. Clicking the chip now opens the record over the page it was clicked from, both on the cards and inside an application's details. Where a page cannot hold a second record open, the chip links through to the prospects page with the row already picked out.
...
* **Filter sessions by summer class**: during the summer course period the session pages show a new Summer button in the toolbar. It narrows the list, week, day and month views by class grade, by Type A or Type B, and by lesson number, and more than one value can be chosen in each row. The button shows what is currently chosen, keeps a count of the chosen values and clears them in one click, so it takes up no more room than a single control. Only the grades, types and lesson numbers actually on screen are offered. Sessions outside the summer course are hidden while a summer filter is on, and when nothing matches the message on screen says so and offers to clear the filter. The button appears only while summer classes are loaded, so the toolbar is unchanged for the rest of the year.


### Bug Fixes

* **Status and tutor filters now appear in the day and month views**: the two filters were only shown in the list and week views, but whatever they were set to still applied in every view. Switching to the day or month view could therefore leave sessions hidden with nothing on screen to explain why. Both filters now appear in all four views, so what is filtered is always visible and can be cleared.
* **A tidier Aged Pending Make-ups view**: opening Aged Pending Make-ups from the notification bell now shows only the controls that list can actually use. The view switcher, date picker, status filter and selection tick boxes no longer appear there, because the list covers its own fixed period and a make-up has to be scheduled before any bulk action can run on it. The tutor filter stays, so the list can still be narrowed to one tutor.
