...
import { Check, ExternalLink, ClipboardCheck } from "lucide-react";
import type {
...
export function StudentHistoryTab() {
  ...
  const tableLabel = (tableId: string) => {
    const t = checktables.find((c) => c.id === tableId);
    return t ? `${t.textbook} ${t.grade} ${t.version}` : tableId;
  };
    ...
    <div className="space-y-4 max-w-3xl">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="text-sm text-ink-500">
          {sorted.length} {sorted.length === 1 ? "entry" : "entries"} across all
          checktables
        </div>
        <div
